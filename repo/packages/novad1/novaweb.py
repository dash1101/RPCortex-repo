...
function cload(){if($('v_codes').style.display=='none')return;api('/codes').then(x=>x.json()).then(l=>{var box=$('cl');box.innerHTML='';if(!l.length){box.appendChild(el('div','muted','no codes'));return}l.forEach(function(o){var d=el('div','net');d.appendChild(el('span',null,o.c+'/'+o.n));var sm=el('small');if(o.c=='ir'||o.c=='subghz'||o.c=='lora'){var rn=el('a',null,'run');rn.href='#';rn.onclick=function(){crun(o.c,o.n);return false};sm.appendChild(rn);sm.appendChild(document.createTextNode(' '))}var ra=el('a',null,'rename');ra.href='#';ra.onclick=function(){cren(o.c,o.n);return false};var da=el('a',null,'del');da.href='#';da.onclick=function(){cdel(o.c,o.n);return false};sm.appendChild(ra);sm.appendChild(document.createTextNode(' '));sm.appendChild(da);d.appendChild(sm);box.appendChild(d)})})}
function crun(c,n){$('um').textContent='running '+n+'...';api('/cmd?c='+encodeURIComponent('novad1 fire '+c+' '+n)).then(x=>x.text()).then(t=>{$('um').textContent=t}).catch(e=>{$('um').textContent=''+e})}
