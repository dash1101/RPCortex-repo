# Desc: RPCMark classic benchmark - RPCortex Vela OS
# File: /Packages/RPCMark/rpcmark_classic.py
# Lang: MicroPython, English
# Version: v3.0.0
# Author: dash1101
#
# The benchmark RPCMark used through v2.1.1, reachable as 'bench classic'.
# It measures mandelbrot, pi and arithmetic throughput, which is a different
# question from the one the default benchmark answers, and its numbers do not
# compare with the v2 build. Kept because it is the only history there is for
# devices already carrying it.
#
# Imported only when asked for, so it costs no import-time RAM otherwise.
# (Formerly "NebulaMark", then "PulseMark".)
...
def run():
    """Entry point used by rpcmark.bench('classic')."""
    RPCMark()
